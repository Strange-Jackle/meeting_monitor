...
const Sidebar: React.FC<SidebarProps> = ({ onNavigateDocs, isOpen, onClose }) => {
  ...
  const mainNavItems = [
    ...
    { label: 'Analytics & Insights', icon: PieChart, path: '/dashboard/analytics' },
  ...
  const appNavItems = [
    ...
    // Mapping Tasks/Scrumboard to existing or placeholder routes for now
    { label: 'Tasks', icon: Settings, path: '/dashboard/settings' },
    { label: 'Scrumboard', icon: LayoutDashboard, path: 'http://localhost:8069', external: true },
  ];
      ...
      >
        ...
        <div className="h-16 flex items-center px-6 border-b border-gray-100 justify-between">
          <div className="flex items-center gap-2 font-bold text-xl text-gray-900 tracking-tight">
            ...
            <span className="font-display font-bold text-xl tracking-tight text-slate-900">Meeting Monitor <span className="text-gray-400 font-normal">AI</span></span>
          </div>
        ...
        <div className="p-4 border-t border-gray-100">
          <div className="flex items-center gap-3 px-2">
            <img src="https://picsum.photos/100/100" alt="User" className="w-8 h-8 rounded-full" />
            <div className="flex-1 overflow-hidden">
              <p className="text-sm font-medium text-gray-900 truncate">Brian Hughes</p>
              <p className="text-xs text-gray-500 truncate">brian@exodus.ai</p>
            </div>
          </div>
        </div>
      </div>
